...
const InvestmentLog: React.FC = () => {
  ...
  // Filter Investments
  const filteredInvestments = mockInvestments.filter((transaction) => {
    const matchesSearch =
  ...
  // Pagination
  const totalPages = Math.ceil(filteredInvestments.length / rowsPerPage)
  const startIndex = (currentPage - 1) * rowsPerPage
  const paginatedInvestments = filteredInvestments.slice(startIndex, startIndex + rowsPerPage)

  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedInvestments(paginatedInvestments.map((t) => t.id))
    } else {
  ...
  const handleSelectTransaction = (transactionId: string, checked: boolean) => {
    if (checked) {
      setSelectedInvestments([...selectedInvestments, transactionId])
    } else {
  ...
  return (
    <div className="space-y-6 bg-background">
      <div>
        ...
        <p className="text-muted-foreground">View and manage all your investment history.</p>
      </div>

      <Card className="bg-card border-border">
        <CardHeader>
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <CardTitle className="text-card-foreground">Investments</CardTitle>
          </div>
        </CardHeader>
        <CardContent>
          {/* Filters */}
                ...
                <TableHeader>
                  <TableRow className="border-border">
                    <TableHead className="w-12">
                      <Checkbox
                        checked={
                          selectedInvestments.length === paginatedInvestments.length && paginatedInvestments.length > 0
                        }
